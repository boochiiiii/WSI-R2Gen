...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_embed, tgt_embed, rm=None):
...
class Decoder(nn.Module):
    ...
    def forward(self, x, hidden_states, src_mask, tgt_mask, memory=None):
        for layer in self.layers:
...
class EncoderDecoder(AttModel):
    ...
    def make_model(self, tgt_vocab):
        ...
        if self.disable_rm:
            model = Transformer(
                ...
                Decoder(
                    DecoderLayerNoRM(self.d_model, c(attn), c(attn), c(ff), self.dropout),
                    self.num_layers),
                lambda x: x,
    ...
    def __init__(self, args, tokenizer):
        ...
        self.disable_rm = bool(getattr(args, 'disable_rm', False))
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks, meshes=None):
        ...
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, meshes=meshes)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        att_feats, seq, att_masks, seq_mask = self._prepare_feature_forward(att_feats, att_masks, seq=seq)
